...
bin_distribution <- function(trials, prob, class = c("bindis", "data.frame")) {
  ...
  return(data.frame(success, probs))
}
...
plot.bindis <- function(trials, prob, ...) {
  probabilities <- bin_distribution(trials, prob)
  barplot(probabilities[2], xlab = "successes", ylab = "probability")
}
...
plot.bincum <- function(trials, prob, ...) {
  plot(bin_cumulative(trials, prob)$cumulative, xlab = "successes", ylab = "probability")
}
